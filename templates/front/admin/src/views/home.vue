<template>
	<div class="home-content">
		...
		<div class="statis-box">
			<div id="statis1" class="statis1 animate__animated" v-if="isAuth('jieshouweixiu','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-zhangjie8"></span>
				</div>
				<div class="right">
					<div class="num">{{jieshouweixiuCount}}</div>
					<div class="name">接收维修总数</div>
				</div>
			</div>
			<div id="statis2" class="statis2 animate__animated" v-if="isAuth('weixiujieguo','首页总数')">
				<div class="left">
					<span class="icon iconfont icon-chujia3"></span>
				</div>
				<div class="right">
					<div class="num">{{weixiujieguoCount}}</div>
					<div class="name">维修结果总数</div>
				</div>
			</div>
		</div>
		<!-- statis -->
		...
		<!-- 2 -->
		<div class="type2">
			<div id="jieshouweixiuChart1" class="echarts1 animate__animated" v-if="isAuth('jieshouweixiu','首页统计')"></div>
			<div id="weixiujieguoChart1" class="echarts2 animate__animated" v-if="isAuth('weixiujieguo','首页统计')"></div>
		</div>
	</div>
</template>
<script>
...
export default {
	...
	mounted(){
		this.init();
		this.getjieshouweixiuCount();
		if(this.isAuth('jieshouweixiu','首页统计')){
			this.jieshouweixiuChat1();
		}
		this.getweixiujieguoCount();
		if(this.isAuth('weixiujieguo','首页统计')){
			this.weixiujieguoChat1();
		}
	...
	methods:{
		handleScroll() {
			let arr = [
				...
				{id:'statis2',css:'animate__fadeInLeft'},
				{id:'news-box',css:'animate__fadeInRight'},
				{id:'jieshouweixiuChart1',css:'animate__fadeInUp'},
				{id:'weixiujieguoChart1',css:'animate__fadeInUp'},
			]
		...
		getjieshouweixiuCount() {
			this.$http({
				url: `jieshouweixiu/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.jieshouweixiuCount = data.data
				}
			})
		},
// 1234 饼
		jieshouweixiuChat1() {
			this.$nextTick(()=>{
				...
				var jieshouweixiuChart1 = echarts.init(document.getElementById("jieshouweixiuChart1"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						jieshouweixiuChart1.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							jieshouweixiuChart1.resize();
						};
					}else{
						this.$message({
							message: data.msg,
		...
		getweixiujieguoCount() {
			this.$http({
				url: `weixiujieguo/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.weixiujieguoCount = data.data
				}
			})
		},
// 1234 竖
		weixiujieguoChat1() {
			this.$nextTick(()=>{
				...
				var weixiujieguoChart1 = echarts.init(document.getElementById("weixiujieguoChart1"),'macarons');
				this.$http({
					...
					if (data && data.code === 0) {
						...
						// 使用刚指定的配置项和数据显示图表。
						weixiujieguoChart1.setOption(option);
				
						//根据窗口的大小变动图表
						window.onresize = function() {
							weixiujieguoChart1.resize();
						};
					}else{
						this.$message({
							message: data.msg,
