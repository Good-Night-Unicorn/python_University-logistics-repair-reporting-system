<template>
	<div class="main-content" :style='{"padding":"20px 30px","textAlign":"center","fontSize":"15px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"10px 0 0","margin":"0 10px 0px","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"0px 0","flexWrap":"wrap","background":"#fff","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('weixiujieguo','维修时长')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						维修时长
					</el-button>
				</el-row>
		...
		<el-dialog
			...
			width="800">
			<div id="weixiushizhangChart1" style="width:100%;height:600px;"></div>
	...
	export default {
		...
		methods: {
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					var weixiushizhangChart1 = echarts.init(document.getElementById("weixiushizhangChart1"),'macarons');
					this.$http({
						url: `weixiujieguo/value/baoxiumingcheng/weixiushizhang`,
						method: "get",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							let res = data.data;
							...
							// 使用刚指定的配置项和数据显示图表。
							weixiushizhangChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								weixiushizhangChart1.resize();
							};
						}else{
							this.$message({
								message: data.msg,
