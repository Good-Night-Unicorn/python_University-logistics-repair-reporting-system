		...
		<el-form
			...
			<el-form-item class="add-item" label="报修单号" prop="baoxiudanhao">
				<el-input v-model="ruleForm.baoxiudanhao" 
					placeholder="报修单号" clearable :disabled=" false  ||ro.baoxiudanhao"></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="维修地点" prop="baoxiudidian">
				<el-input v-model="ruleForm.baoxiudidian" 
					placeholder="维修地点" clearable :disabled=" false  ||ro.baoxiudidian"></el-input>
			</el-form-item>
			...
			<el-form-item class="add-item" label="完成时间" prop="wanchengshijian">
				<el-date-picker
					:disabled="true  ||ro.wanchengshijian"
					value-format="yyyy-MM-dd HH:mm:ss"
			...
			<el-form-item class="add-item" label="维修时长" prop="weixiushizhang">
				<el-input v-model.number="ruleForm.weixiushizhang" 
					placeholder="维修时长" clearable :disabled=" false  ||ro.weixiushizhang"></el-input>
			</el-form-item>
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			info() {
				this.$http.get(`weixiujieguo/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`weixiujieguo/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
