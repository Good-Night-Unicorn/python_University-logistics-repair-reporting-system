	...
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item"  label="报修类型" prop="baoxiuleixing">
				<el-select v-model="ruleForm.baoxiuleixing" placeholder="请选择报修类型" :disabled=" false  ||ro.baoxiuleixing" >
					<el-option
			...
			<el-form-item class="add-item"  label="审批状态" prop="shenpizhuangtai">
				<el-select v-model="ruleForm.shenpizhuangtai" placeholder="请选择审批状态" :disabled="true  ||ro.shenpizhuangtai" >
					<el-option
	...
	export default {
		...
		methods: {
			...
			init(type) {
				...
				this.$http.get('option/baoxiuleixing/baoxiuleixing', {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
						this.baoxiuleixingOptions = res.data.data;
					}
				});
			...
			// 多级联动参数
			info() {
				this.$http.get(`baoxiushenqing/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`baoxiushenqing/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
