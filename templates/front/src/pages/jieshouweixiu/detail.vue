	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="'·'">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/jieshouweixiu?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('jieshouweixiu','修改')" @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('jieshouweixiu','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('jieshouweixiu','结果')" 
							@click="onAcross('weixiujieguo','','','weixiuzhuangtai','维修已完成！','已完成,维修中'.split(',')[0])" type="warning">
							结果
						</el-button>
					</div>
				</div>
	...
	export default {
		...
		methods: {
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				localStorage.setItem('crossTable',`jieshouweixiu`);
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/jieshouweixiu', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/jieshouweixiuAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此接收维修？') .then(_ => {
					this.$http.post('jieshouweixiu/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
