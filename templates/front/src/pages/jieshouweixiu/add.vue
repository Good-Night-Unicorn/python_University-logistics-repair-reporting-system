	...
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="接收时间" prop="jieshoushijian">
				<el-date-picker
					:disabled="true  ||ro.jieshoushijian"
	...
	export default {
		...
		methods: {
			...
			init(type) {
				...
				this.weixiuzhuangtaiOptions = "已完成,维修中".split(',')
				this.$http.get('option/weixiurenyuan/weixiuzhanghao', {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			weixiuzhanghaoChange () {
				this.$http.get('follow/weixiurenyuan/weixiuzhanghao?columnValue=' + this.ruleForm.weixiuzhanghao, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			// 多级联动参数
			info() {
				this.$http.get(`jieshouweixiu/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`jieshouweixiu/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
